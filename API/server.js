...
app.get("/stock/:id/details", (req, res) => {
  const stockId = req.params.id;
  const query = `
    SELECT stockitem_id, engine_no, chassis_no, battery_no, pcb_no, charger_serial_no, hsn_no, qty, create_date
    FROM stock_items
    WHERE stock_id = ? 
...
app.post("/stock_items", (req, res) => {
  try {
    ...
    if (!stock_id) {
      return res.status(400).json({ message: "Missing stock_id" });
    }

    const safeStr = (v) =>
      v === undefined || v === null || v === "" ? null : v.toString().trim();
    ...
    const values = [
      ...
      safeStr(pcb_no),
      safeStr(chassis_no),
      safeStr(charger_serial_no),
      safeStr(hsn_no),
    ];

    const checkStockQuery = `SELECT sid, available_qty FROM stock WHERE sid = ?`;
    db.query(checkStockQuery, [stock_id], (err, result) => {
      if (err) {
      ...
      db.query(insertQuery, values, (error, insertResult) => {
        if (error) {
          console.error("❌ Error inserting stock item:", error);
          return res
            .status(500)
            .json({ message: "Failed to insert stock item", error });
        }

        // Update available_qty in stock table
        const updateStockQuery = `
          UPDATE stock 
          SET available_qty = available_qty + 1, updated_at = NOW()
          WHERE sid = ?
        `;
        db.query(updateStockQuery, [stock_id], (updateErr) => {
          if (updateErr) {
            console.error(
              "⚠️ Failed to update stock available_qty:",
              updateErr
            );
            // Not fatal, proceed to return inserted data
          }
          ...
          const joinQuery = `
            SELECT 
              ...
              si.pcb_no,
              si.chassis_no,
              si.charger_serial_no,
              si.hsn_no, 
              si.create_date
            FROM stock_items si
            INNER JOIN stock s ON s.sid = si.stock_id
          ...
          db.query(
            ...
            (joinErr, joinedData) => {
              if (joinErr) {
                console.error("⚠️ Join fetch failed:", joinErr);
                return res.status(201).json({
                  message: "Stock item added successfully (no join data)",
